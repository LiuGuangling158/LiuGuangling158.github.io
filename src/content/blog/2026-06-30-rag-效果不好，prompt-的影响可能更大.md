...
即使 Prompt 里写了"请严格输出 JSON 格式"，LLM 有时候还是会在 JSON 外面包一层 markdown 代码块。所以 JSON 解析必须做两重 fallback：先试直接解析，失败了再从 markdown 代码块里提取，再失败就用原始文本兜底。

这不是 Prompt 写得不够好——而是 LLM 的非确定性决定了必须防御性编程。